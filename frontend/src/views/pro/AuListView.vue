...
interface Auc { id: number; auctionNo: string; auctionTitle: string; startPrc?: number; awardVdNm?: string; awardAmt?: number; startYmd?: string; sts: string; stsNm?: string }
...
async function load() { list.value = (await http.get('/pro/au', { params: { keyword: keyword.value, sts: sts.value } })).data.data }
async function loadCodes() { codes.value = (await http.get('/sys/code/codes', { params: { grpCd: 'AUCTION_STS' } })).data.data }
function open(id: number | 'new') { router.push(`/pro/au/${id}`) }
...
<template>
  <div class="page">
    <h2>역경매</h2>
    <div class="toolbar">
      <input v-model="keyword" placeholder="경매번호/제목" @keyup.enter="load" />
      ...
      <button class="btn primary" @click="open('new')">+ 신규 역경매</button>
    </div>
    <div class="panel">
      <table>
        <thead><tr><th>경매번호</th><th>제목</th><th class="r">시작가</th><th>낙찰협력사</th><th class="r">낙찰가</th><th>상태</th></tr></thead>
        <tbody>
          <tr v-for="a in list" :key="a.id" class="row" @click="open(a.id)">
            <td>{{ a.auctionNo }}</td><td>{{ a.auctionTitle }}</td><td class="r">{{ fmt(a.startPrc) }}</td>
            <td>{{ a.awardVdNm }}</td><td class="r">{{ a.awardAmt ? fmt(a.awardAmt) : '-' }}</td>
            <td><span class="badge" :class="a.sts">{{ a.stsNm }}</span></td>
          </tr>
          <tr v-if="!list.length"><td colspan="6" class="empty">역경매가 없습니다.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
.badge.AWARD { background: #e3f6e8; color: #1a7f37; } .badge.CANCEL { background: #f3e3e3; color: #a33; }
</style>
