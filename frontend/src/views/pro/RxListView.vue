...
interface Rfx { id: number; rfxNo: string; rfxTitle: string; selVdNm?: string; openYmd?: string; closeYmd?: string; sts: string; stsNm?: string }
...
async function load() {
  const { data } = await http.get('/pro/rx', { params: { keyword: keyword.value, sts: sts.value } })
  list.value = data.data
}
async function loadCodes() { codes.value = (await http.get('/sys/code/codes', { params: { grpCd: 'RFX_STS' } })).data.data }
function open(id: number | 'new') { router.push(`/pro/rx/${id}`) }
...
<template>
  <div class="page">
    <h2>견적</h2>
    <div class="toolbar">
      <input v-model="keyword" placeholder="견적번호/제목" @keyup.enter="load" />
      ...
      <button class="btn primary" @click="open('new')">+ 신규 견적</button>
    </div>
    <div class="panel">
      <table>
        <thead><tr><th>견적번호</th><th>제목</th><th>선정협력사</th><th>공고일</th><th>마감일</th><th>상태</th></tr></thead>
        <tbody>
          <tr v-for="r in list" :key="r.id" class="row" @click="open(r.id)">
            <td>{{ r.rfxNo }}</td><td>{{ r.rfxTitle }}</td><td>{{ r.selVdNm }}</td>
            <td>{{ r.openYmd }}</td><td>{{ r.closeYmd }}</td>
            <td><span class="badge" :class="r.sts">{{ r.stsNm }}</span></td>
          </tr>
          <tr v-if="!list.length"><td colspan="6" class="empty">견적이 없습니다.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
