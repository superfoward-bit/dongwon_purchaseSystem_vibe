...
interface Ct { id: number; ctNo: string; ctTitle: string; vdNm?: string; validSd?: string; validEd?: string; totAmt?: number; sts: string; stsNm?: string }
...
async function load() { list.value = (await http.get('/pro/ct', { params: { keyword: keyword.value, sts: sts.value } })).data.data }
async function loadCodes() { codes.value = (await http.get('/sys/code/codes', { params: { grpCd: 'CT_STS' } })).data.data }
function open(id: number | 'new') { router.push(`/pro/ct/${id}`) }
...
<template>
  <div class="page">
    ...
    <div class="panel">
      <table>
        <thead><tr><th>계약번호</th><th>제목</th><th>협력사</th><th>유효기간</th><th class="r">계약금액</th><th>상태</th></tr></thead>
        <tbody>
          <tr v-for="c in list" :key="c.id" class="row" @click="open(c.id)">
            <td>{{ c.ctNo }}</td><td>{{ c.ctTitle }}</td><td>{{ c.vdNm }}</td>
            <td>{{ c.validSd }} ~ {{ c.validEd }}</td><td class="r">{{ fmt(c.totAmt) }}</td>
            <td><span class="badge" :class="c.sts">{{ c.stsNm }}</span></td>
          </tr>
          <tr v-if="!list.length"><td colspan="6" class="empty">계약이 없습니다.</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
.badge.ACTIVE { background: #e3f6e8; color: #1a7f37; } .badge.EXPIRE { background: #eee; color: #777; }
.badge.TERMINATE { background: #f3e3e3; color: #a33; } .badge.CANCEL { background: #f3e3e3; color: #a33; }
</style>
